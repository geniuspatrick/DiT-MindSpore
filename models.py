...
from mindspore.common.initializer import Constant, XavierNormal, XavierUniform, Normal, One, Zero, initializer
...
class PatchEmbed(nn.Cell):
    ...
    def construct(self, x: Tensor) -> Tensor:
        ...
        if self.image_size is not None:
            assert (h, w) == (self.image_size[0], self.image_size[1]), f"Input height and width ({h},{w}) doesn't match model ({self.image_size[0]},{self.image_size[1]})."
        x = self.proj(x)
...
class Attention(nn.Cell):
    ...
    def construct(self, x):
        ...
        q, k, v = ops.unstack(qkv)

        q, k = self.q_norm(q), self.k_norm(k)
        attn = self.q_matmul_k(q, k)
        attn = ops.mul(attn, self.scale)

        origin_dtype = attn.dtype
        attn = attn.astype(ms.float32)
        attn = ops.softmax(attn, axis=-1)
        attn = attn.astype(origin_dtype)
